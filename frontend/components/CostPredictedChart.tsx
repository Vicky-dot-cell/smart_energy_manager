...
export function CostPredictedChart() {
    ...
    return (
        <>
            ...
            >
                <h3 className="text-sm font-medium text-gray-400 uppercase tracking-wider mb-4 group-hover:text-gray-300 transition-colors">Cost Predicted</h3>
            ...
            <ChartModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Cost Predicted">
                <div className="w-full h-full relative flex items-center justify-center">
                    <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none pb-4 z-10">
                        ...
                        <span className="text-5xl font-bold text-white mt-2 pb-8">{formatCurrency(totalCost)}</span>
                    </div>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            ...
                            <Tooltip contentStyle={{ backgroundColor: '#171717', borderRadius: '8px', border: '1px solid #262626', color: '#fff' }} itemStyle={{ color: '#fff' }} formatter={(value: any) => [formatCurrency(value), 'Cost']} />
                            <Legend verticalAlign="bottom" align="center" iconType="circle" iconSize={12} height={40} formatter={(value) => <span className="text-gray-400 ml-2 text-sm uppercase font-medium tracking-wider">{value}</span>} />
                        </PieChart>
                    </ResponsiveContainer>
                </div>
            </ChartModal>
